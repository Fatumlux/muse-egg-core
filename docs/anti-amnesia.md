...
`ContextWindowEngine` 保存最近幾輪事件與 OC 回應，專門處理「剛才、上面、那個、繼續」這類短期指涉。

短期上下文不等於長期記憶。它會隨 runtime 設定修剪，並在送給 provider 前加上本機時間、生命狀態與越權提醒。真正需要跨重啟保存的內容仍由 `MemoryEngine` 與 continuity ledger 寫入。
...
MuseEgg Core 不能保證永遠不出錯，但設計原則是：原始事件不丟、短期上下文不擠掉長期身份、記憶自動落盤、重啟可恢復、LLM 不擁有記憶權限。
